...
plot_all   = opts.plot_all;   % <- new local flag
...
% Mean ± SEM across subjects for the selected component
if nsubs > 1
...
% Plot mean eigenspectrum with optional SEM band
hold on
if nsubs > 1 && ~all(isnan(sem_eig))
    fill([x; flipud(x)], [avg_eig - sem_eig; flipud(avg_eig + sem_eig)], ...
        [0 0 0], 'FaceAlpha', 0.1, 'EdgeColor', 'none');
end
plot(x, avg_eig, 'k', 'LineWidth', 2);

% Plot fitted exponential decay within the fitted range
...
% Add dashed x-lines to mark the fitted frequency range
x_min_fit = x(idx_range2fit(1));
x_max_fit = x(idx_range2fit(end));
xline(x_min_fit, 'k--', 'LineWidth', 1);
...
if plot_all
    for subi = 1:nsubs
        ...
        plot(x, y_full, 'k', 'LineWidth', 1.5);
        
        % Subject-specific exponential fit within the fitted range, if available
        ...
        % Dashed lines delimiting the fitted frequency range
        xline(x_min_fit, 'k--', 'LineWidth', 1);
        xline(x_max_fit, 'k--', 'LineWidth', 1);
