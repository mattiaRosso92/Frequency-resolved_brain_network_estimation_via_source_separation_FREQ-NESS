...
old = warning('off'); resetWarn = onCleanup(@()warning(old)); %#ok<NASGU> % MATLAB:structOnObject
...
try
    ...
    if regexp(me.message, 'Unrecognized.+CEF', 'once') % >=R2025a
        error("For Matlab R2025a or later, add the following line before creating the uifigure:" + newline +...
            "try addprop(groot, 'ForceIndependentlyHostedFigures'); catch, end");
    elseif regexp(me.message, 'Unrecognized.+enableDragAndDropAll', 'once') % <R2020b
...
for i = size(h.UserData,1):-1:1 % redo in case pos changed or resized
    obj = h.UserData{i,2};
    p{i} = round(getpixelposition(obj, true)); %#ok<AGROW>
