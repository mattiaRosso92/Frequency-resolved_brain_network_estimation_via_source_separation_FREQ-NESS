...
%% ADVANCED SCIENCE - REVISION I

%%

addpath('/Users/au550322/Documents/AarhusUniversitet/MattiaRosso/Paper_FREQNESS_GED')

%% SUPPLEMENTARY FIGURES - FREQUENCY LANDSCAPE OF EXPLAINED VARIANCE

col_l = 1; %1 for significant time-windows with different colors; 0 = only grey color
ylimm = [3 21]; %the following is the limit for the main figure of the actual data for the paper [2.5 21] %amplitude limits; leave empty [] for automatic adjustment
sbrim = -0.01; %number to adjust the placement of the lines/markers for significance
Bonf_thresh = 0; %1 for Bonferroni correction, 0 for FDR correction
lineplotdum = [20 2];
inter_l = 1; %1 = interpolation of frequencies around power line artifacts; 0 = no interpolation
load_realdata = 8; %define the case you want (refer to the variable "list")

frex = 1:39;
task_l = 2; %1 = resting state; 2 = beat
comps = [1:3]; %1 = Old; 2 = NewT1; 3 = NewT2; 4 = NewT3; 5 = NewT4;
workingdir = '/Users/au550322/Documents/AarhusUniversitet/MattiaRosso/Paper_FREQNESS_GED/Mattia';

close all

list = dir('/Users/au550322/Documents/AarhusUniversitet/MattiaRosso/Paper_FREQNESS_GED/AdvancedSciences_Revision_I/Figures/*.mat');
% if load_realdata == 1
    load([list(1).folder '/' list(load_realdata).name])
    Pevals(:,:) = 0.5; %simplistic way to code no indication of significance
% end
...
% if ndims(evals_all) == 3
%     SS = size(evals_all);
%     evals_all_dum = zeros(SS(1),SS(2),SS(3),1);
%     evals_all_dum(:,:,:,1) = evals_all;
%     evals_all = evals_all_dum;
% end
...
%     S.signtp_col = [];
% end
title([list(load_realdata).name '_Cond' num2str(task_l)])

waveform_plotting_local_v3(S) %actual function

exportgraphics(gcf,[list(1).folder '/' list(load_realdata).name(1:end-4) '_Cond' num2str(task_l) '.pdf'],'Resolution',300)

%%

%% SUPPLEMENTARY FIGURE: EXPONENTIAL DECAY OF EXPLAINED VARIANCE

% directory and file name
load('/Users/au550322/Documents/AarhusUniversitet/MattiaRosso/Paper_FREQNESS_GED/AdvancedSciences_Revision_I/GEDallsubjs.mat')

idx2plot = [7 9 13];
nfrex = length(idx2plot);
nsubs = size(evals_all,2);
ncomps = 30;
nconds = 2;
espectrum = zeros(ncomps, nsubs, nfrex, nconds); 
...
for frexi = 1:3
    figure
    espectrum(:,:,frexi,:) = evals_all(1:ncomps, :, idx2plot(frexi), :);
    avg_espectrum = squeeze(mean(espectrum,2));
    std_espectrum = squeeze(std(espectrum,[],2)./sqrt(size(evals_all,2)));
    
%     plot(avg_espectrum(:,frexi,1),'s-','markerfacecolor','k','linew',linewidth), hold on
    errorbar(avg_espectrum(:,frexi,1),std_espectrum(:,frexi,1),'s-','markerfacecolor','k','linew',linewidth), hold on
    errorbar(avg_espectrum(:,frexi,2),std_espectrum(:,frexi,2),'s-','markerfacecolor','k','linew',linewidth)
%     plot(avg_espectrum(:,frexi,2),'s-','markerfacecolor','k','linew',linewidth)    

    xlabel('Component #'), ylabel('Explained variance (%)')
    title(['Eigenspectrum for ' num2str(all_frex(idx2plot(frexi))) ' Hz'])
    ylim([0 2+max(max(avg_espectrum(:,frexi,:)))])
    %legend({'RS','PL'})
    axis square
    ...
    exportgraphics(gcf,['/Users/au550322/Documents/AarhusUniversitet/MattiaRosso/Paper_FREQNESS_GED/AdvancedSciences_Revision_I/' num2str(frexi) '.pdf'],'Resolution',300)
