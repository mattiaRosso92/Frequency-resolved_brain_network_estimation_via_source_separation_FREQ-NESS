...
for subi = 12%:length(list) %over subjects
    S = [];
...
S.path_GEDoutput = '/scratch7/MINDLAB2017_MEG-LearningBach/Mattia/after_maxfilter_mc/Source_LBPD/Paper_GED/Revisions/Duration/';
S.name_folder    = ['GEDoutput_frex_' num2str(S.targetfrex) 'Hz' cond_suffix{which_cond} '_' num2str(S.time) '_sec/']; 
...
%% CLUSTER-BASED TEST FOR REVISION I - ADVANCED SCIENCE

xxx = 1; %number of component
P = Pevals(xxx,:);
T = Tevals(xxx,:);
[ sign_clust ] = oneD_MCS_LBPD_D( P, 0.05, 0, 1000, 0.01, all_frex, T ) %Monte Carlo simulation function to correct for multiple comparisons
