...
%% PCA Assessment (Randomizations of Rest)
...
for frexi = 1:length(freq_vect_idx)
    ...
    for randi = 1:nrands
        ...
        for subi = 1:nsubs % over subjects
            ...
            disp(['Randomization: ' rand_names{randi} ' - PCA assessment of freq idx #' num2str(frexi) ' - participant #' num2str(subi)])
        end
        ...
        for compi = comps2see % over components of interest
            ...
            %     else
            if outliers_flag ==1
                save_nii(nii,[path_PCAoutput '/SubjsAverage_Comp_' num2str(compi) '_Var_' num2str(round(evals_avg(compi))) 'frex_' num2str(list_rest(freq_vect_idx(frexi)).name(16:end)) '_NO_outliers.nii']); % printing image
            else
                save_nii(nii,[path_PCAoutput '/SubjsAverage_Comp_' num2str(compi) '_Var_' num2str(round(evals_avg(compi))) 'frex_' num2str(list_rest(freq_vect_idx(frexi)).name(16:end)) '.nii']); % printing image
            end
        end
    end
end
...
%         for voxi = 1:nvoxels
% 
%             %[~,p,~,stats] = ttest( squeeze(PCAmaps_rest(compi,voxi,:,frexi)) , squeeze(PCAmaps_beat(compi,voxi,:,frexi)) );
%             [p,~,stats] = signrank( squeeze(PCAmaps_rest(compi,voxi,:,frexi)) , squeeze(PCAmaps_beat(compi,voxi,:,frexi)) );
...
% subplot(311)
% plot(squeeze( PCAmaps_rest(comp2see,:,:,idx_frex ) ) , 'k'), hold on
% plot(squeeze( PCAmaps_beat(comp2see,:,:,idx_frex) ) , 'r')
% line([stimfrex stimfrex], get(gca, 'Ylim'), 'color', 'r', 'LineStyle', '--')
% subplot(312)
...
path_PCAoutput = '/scratch7/MINDLAB2017_MEG-LearningBach/Mattia/after_maxfilter_mc/Source_LBPD/Paper_GED';
% Get all_frex from home path
list_PCA = dir([path_PCAoutput '/PCA*']); % keep only folders

load('/scratch7/MINDLAB2017_MEG-LearningBach/Mattia/after_maxfilter_mc/Source_LBPD/Paper_GED/Subjs_idx_match.mat');
for condi = 1:2
    list_PCA_cond = list_PCA(endsWith({list_PCA.name}, ['Hz', suffix{condi}])); % only ending with 'Hz'
    % Initialize vectors for storing frequencies
    nfrex = length(list_PCA_cond);
    all_frex = zeros(nfrex,1);
    % Retrieve and store the frequency
    for frexi = 1:numel(list_PCA_cond)
        % Check if the folder name contains 'Hz'\
        this_frex = regexp(list_PCA_cond(frexi).name, '([\d.]+)Hz', 'tokens');
        all_frex(frexi) = str2double (this_frex{1,1});
    end
    % Re-sort vector of frequencies (strings follow alphabetic order)
    [all_frex, idx_sort] = sort(all_frex, 'ascend');
    ...
    % Re-compute fwhm for all frequencies (NOTE: should be the same used to perform PCA)
    ...
    this_folder =  [path_PCAoutput, '/PCAoutput_frex_' num2str(lfo_frex) 'Hz' suffix{condi} '/']; % Get folder name
    ...
    for subi = 1:nsubs % over subjects
        ...
        load([this_folder, list_subs(subi).name],'PCAts')
        lfo_ts  = PCAts(comps2see,:); % only keep relevant components
        % Filter timeseries
    ...
    for frexi = 1:nfrex
        ...
        % Get folder name
        this_folder =  [path_PCAoutput,'/PCAoutput_frex_' num2str(all_frex(frexi)) 'Hz' suffix{condi} '/'];
        ...
        for subi = 1:nsubs % over subjects
            ...
            % Get components timeseries (already sorted and normalized)
            load([this_folder, list_subs(subi).name],'PCAts')
            carr_ts  = PCAts(comps2see,:); % only keep relevant components
            % Filter timeseries
...
dcOff(dcOff   == 0) = NaN;
% Save outputs
save([path_PCAoutput '/CFC_by_' num2str(lfo_frex) '_comp#' num2str(which_lfo) '.mat'], 'PAC_all','PAC_avg','MPA_all','MPA_avg','sAmpl','pShift','dcOff','mFrex','goodFit','all_frex','phase_edges');
...
% Load CFC data
load([path_PCAoutput '/CFC_by_' num2str(lfo_frex) '_comp#' num2str(which_lfo) '.mat']);
...
path_PCAoutput = '/scratch7/MINDLAB2017_MEG-LearningBach/Mattia/after_maxfilter_mc/Source_LBPD/Paper_GED/';
% Import data
load([path_PCAoutput '/CFC_by_' num2str(lfo_frex) '_comp#' num2str(which_lfo) '.mat'], 'PAC_all','PAC_avg','MPA_all','MPA_avg','sAmpl','pShift','dcOff','mFrex','goodFit','all_frex','phase_edges');
