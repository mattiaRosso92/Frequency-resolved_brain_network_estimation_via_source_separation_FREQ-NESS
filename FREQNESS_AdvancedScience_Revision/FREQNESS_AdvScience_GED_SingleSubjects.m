function O = FREQNESS_AdvScience_GED_SingleSubjects(S)

% NOTE: 'duration' and 'pnts2keep' are the only addition to the original
% 'GED_SingleSubjects_v3.m' script; these are used to match the size of the
% original FREQNESS dataset when importing the source reconstructed data

O = []; 

% Settings (assign from input structure)
srate      = S.srate;       % recorded at 1000 Hz, downsampled by 4
time   = S.time; % in seconds (to match the original dataset)
pnts2keep  = time * srate;
...
for subi = 1:length(list)
    ...
    broadData = OUT.sources_ERFs(:,1:pnts2keep);
